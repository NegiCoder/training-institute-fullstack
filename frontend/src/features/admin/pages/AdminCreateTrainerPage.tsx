...
export function AdminCreateTrainerPage() {
  const [createdTrainer, setCreatedTrainer] = useState<AuthResponse | null>(null)
  ...
  async function onSubmit(values: CreateTrainerFormValues) {
    ...
    try {
      const response = await authService.createTrainer({
      ...
      <form className="auth-form" onSubmit={handleSubmit(onSubmit)}>
        ...
        {createdTrainer && (
          <div className="alert success-alert">
            Trainer created. User ID: {createdTrainer.userId}. Email:{' '}
            {createdTrainer.email}
          </div>
        )}
