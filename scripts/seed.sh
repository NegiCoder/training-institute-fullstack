#!/usr/bin/env bash
# =============================================================================
# Training Institute - DB Seed Script
# =============================================================================
# What it does:
#   1. Wipes all data from the DB (keeps schema)
#   2. Creates 1 admin, 3 trainers, 5 students (via API so passwords are BCrypt)
#   3. Creates 5 categories, 6 courses, pricing, modules, trainer assignments
#   4. Creates several enrollments, marks some module progress
#   5. Updates statuses, issues 1 certificate
#
# Requirements:
#   - Backend running on http://localhost:5045
#   - SQL Server container named "training-sql"
#   - jq + curl installed
#
...
SQL_CONTAINER="training-sql"
SQL_USER="sa"
SQL_PASS="${SQL_PASS:?Set SQL_PASS before running, for example: SQL_PASS='your-local-sa-password' bash scripts/seed.sh}"
SQL_DB="TrainingInstitute"
...
sql() {
  docker exec -i "$SQL_CONTAINER" /opt/mssql-tools18/bin/sqlcmd \
    -S localhost -U "$SQL_USER" -P "$SQL_PASS" -d "$SQL_DB" -C -b -h -1 -Q "$1"
}
...
curl -fs "$API/swagger/index.html" >/dev/null || fail "Backend not reachable at $API"
ok "Backend is reachable at $API"
docker exec "$SQL_CONTAINER" /opt/mssql-tools18/bin/sqlcmd \
  -S localhost -U "$SQL_USER" -P "$SQL_PASS" -d "$SQL_DB" -C -Q "SELECT 1" >/dev/null \
  || fail "SQL container not reachable"
ok "SQL Server is reachable"
...
" >/dev/null
ok "All tables emptied and identities reset"

# ---------- step 2: bootstrap admin -----------------------------------------

hdr "Step 2: Bootstrap admin user"
register_student "Admin Boss" "$ADMIN_EMAIL" "$ADMIN_PASS"
sql "UPDATE Users SET Role = 3, FullName = 'Admin Boss' WHERE Email = '$ADMIN_EMAIL'" >/dev/null
ADMIN_TOKEN=$(login "$ADMIN_EMAIL" "$ADMIN_PASS")
[[ -n "$ADMIN_TOKEN" && "$ADMIN_TOKEN" != "null" ]] || fail "Admin login failed"
ok "Admin created and logged in"

# ---------- step 3: trainers ------------------------------------------------

hdr "Step 3: Creating 3 trainers"
...
hdr "Step 4: Creating 5 students with profiles"
...
hdr "Step 5: Creating 5 categories"
...
# ---------- step 6: courses + pricing + modules + trainers ------------------

hdr "Step 6: Creating 6 courses (with pricing, modules, trainer assignments)"

# status: 1=Draft, 2=Published, 3=Archived
# contentType: 1=Video, 2=Pdf, 3=Link

# NOTE: order matters now - admin creates course + pricing + assigns trainer first,
# THEN the assigned trainer logs in and uploads modules (only trainers can manage content)
...
hdr "Step 7: Creating enrollments"
...
# Karan on React: 2 of 5 modules done
mark_module_complete "$E_S5_C3" "$C3M1" "$S5"
mark_module_complete "$E_S5_C3" "$C3M2" "$S5"

ok "Progress recorded"

# ---------- step 9: admin status updates -----------------------------------

hdr "Step 9: Admin moves enrollments through lifecycle"

# 2 = InProgress, 3 = Completed
